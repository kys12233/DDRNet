...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=11, planes=64, spp_planes=128, head_planes=128, augment=True):
...
def DualResNet_imagenet(cfg, pretrained=False):
    ...
    # augment=True) # ori
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=11, planes=32, spp_planes=128, head_planes=64,
                       augment=True)
...
def get_seg_model(cfg, **kwargs):
    ...
    # model = DualResNet_imagenet(cfg, pretrained=True) # ori
    model = DualResNet_imagenet(cfg, pretrained=False) # 先不做预训练
    return model


if __name__ == '__main__':
    x = torch.rand(4, 3, 800, 800)
    # net = DualResNet_imagenet(pretrained=True) #ori
    cfg = '1'
    net = DualResNet_imagenet(cfg)
    y = net(x)
    print(len(y))
    # exit()
    print(y[0].shape)
