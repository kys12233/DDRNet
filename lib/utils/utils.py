...
def get_confusion_matrix(label, pred, size, num_class, ignore=-1):
    ...
    # print("seg_pred shape is :", seg_pred.shape)
    # print("seg_gt shape is : ",seg_gt.shape)
    # exit()
    ignore_index = seg_gt != ignore # 判断标签中是否含有ignore,对应的每个位置返回Ttue或者False

    # print(ignore_index)
    # print(ignore_index.shape)
    # exit()
    # print(seg_gt.shape)
    ...
    # exit()
    # print(num_class)
    # print((seg_gt * num_class + seg_pred).shape)
    index = (seg_gt * num_class + seg_pred).astype('int32')
    # index = (seg_gt * num_class).astype('int32')
    label_count = np.bincount(index) # 计算数据出现的次数
    # print(label_count)
