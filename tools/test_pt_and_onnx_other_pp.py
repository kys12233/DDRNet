...
def pth_to_onnx(input, pth_path, onnx_path, input_names=['input'], output_names=['output'], device='cpu'):
    ...
    torch.onnx.export(model, input, onnx_path, verbose=True, opset_version=11, input_names=input_names, output_names=output_names)
...
def get_image(image_path):
    ...
    def resize_padding(image, target_length, value=0):
        ...
        dw, dh = (iw - nw) // 2, (i h -nh) // 2
        image_paded.paste(image_resized, (dh, dw, n h +dh, n w +dw))   # 居中填充图片
        return image_paded
