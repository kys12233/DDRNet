...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        # default="experiments/cityscapes/ddrnet_slim.yaml", #ori
...
def main():
    ...
    if config.LOSS.USE_OHEM:
        # OhemCrossEntropy是改进版的CrossEntropy，有利于处理困难样本
    ...
    # model = FullModel(model, criterion) # 不把model的loss函数合并
    # print("ok")
    # exit()
    # if distributed:
    ...
    # print(config.TRAIN.BATCH_SIZE_PER_GPU)
    # print(len(gpus))


    # epoch_iters = np.int(train_dataset.__len__() /
    #                     config.TRAIN.BATCH_SIZE_PER_GPU / len(gpus)) #ori
    epoch_iters = len(train_dataset)
    # print(epoch_iters)
    # exit()
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if epoch >= config.TRAIN.END_EPOCH:
            ...
            #       trainloader, optimizer, model, writer_dict) # ori
            get_loss = train(config, epoch, config.TRAIN.END_EPOCH,
                  epoch_iters, config.TRAIN.LR, num_iters,
                  trainloader, optimizer, model, criterion, loss_my_train_01)

        # if epoch % 10 == 0:
        #     valid_loss, mean_IoU, IoU_array = validate(config,
        #                 testloader, model, writer_dict) #ori
        # with open("train_loss.txt", "a") as file:
        #     file.write("{}      {}".format(epoch,get_loss) + '\n')
        ...
        # }, 'checkpoint.pth.tar')
        # torch.save(model.state_dict(), 'save_{}.pt'.format(epoch))
        # exit()
